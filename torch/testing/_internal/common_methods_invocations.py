...
op_db: List[OpInfo] = [
    ...
    OpInfo('linalg.lu_factor',
           ...
           skips=(
               ...
               DecorateInfo(unittest.skip("memory hog"), 'TestGradients', 'test_fn_gradgrad',
                            device_type='cuda', dtypes=[torch.complex128, torch.float64]),
           )),
    ...
    OpInfo('linalg.lu',
           ...
           skips=(
               ...
               DecorateInfo(unittest.skip("memory hog"), 'TestGradients', 'test_fn_gradgrad',
                            device_type='cuda', dtypes=[torch.complex128, torch.float64]),
               DecorateInfo(unittest.skip("memory hog"), 'TestGradients', 'test_fn_grad',
                            device_type='cuda', dtypes=[torch.complex128]),
               DecorateInfo(unittest.skip("memory hog"), 'TestGradients', 'test_forward_mode_AD',
                            device_type='cuda', dtypes=[torch.complex128]),
           )),
    OpInfo('lu_unpack',
           op=torch.lu_unpack,
